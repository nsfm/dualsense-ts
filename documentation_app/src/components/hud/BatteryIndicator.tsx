...
function batteryIntent(level: number, status: ChargeStatus): TagIntent {
  if (status === ChargeStatus.Charging || status === ChargeStatus.Full) {
    return "success";
  }
  if (level < 0.2) return "danger";
...
export const BatteryIndicator = () => {
  ...
  return (
    <BatteryContainer>
      <Tag $minimal $intent={batteryIntent(level, status)}>
        {batteryIcon} {chargeLabel(status)}: {pct}%
      </Tag>
    </BatteryContainer>
  );
};
